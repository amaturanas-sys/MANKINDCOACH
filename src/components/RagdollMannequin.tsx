...
import {
  Rig, Pose, RigPart, buildIndex, spriteMatrix, worldOf, pivotScreen,
  toSvgMatrix, rgb
} from '../lib/ragdoll';
  ...
  return (
    <svg ref={ref} viewBox={viewBox} className={className} style={{ touchAction: 'none', userSelect: 'none' }}>
      ...
      {interactive && selectedId && (() => {
        ...
        const [x, y, w, h] = p.rect;
        return (
          <rect x={x} y={y} width={w} height={h} transform={toSvgMatrix(spriteMatrix(rig, pose, p, byId))}
      ...
      {interactive && showJoints && sprites.map(p => {
        ...
        const [cx, cy] = pivotScreen(rig, pose, p, byId);
